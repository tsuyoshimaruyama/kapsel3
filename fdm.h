...
inline int adj(int x, int i, int NX) {
	return ((i + x) < 0) ? i + x + NX : (((i + x) > NX - 1) ? i + x - NX : i + x);
}

inline int ijk2im(int i, int j, int k) {
	return (i * NY * NZ_) + (j * NZ_) + k;
}
...
inline double calc_gradient_o1_to_o2(const double * field, int im, int dir) {
	...
	case 0:
		retval = (
			field[ijk2im(ip1, jp1, kp1)] - field[ijk2im(i, jp1, kp1)]
			+ field[ijk2im(ip1, jp1, k)] - field[ijk2im(i, jp1, k)]
			+ field[ijk2im(ip1, j, kp1)] - field[ijk2im(i, j, kp1)]
			+ field[ijk2im(ip1, j, k)] - field[im]
			) * INV_4DX;
		break;
	case 1:
		retval = (
			field[ijk2im(ip1, jp1, kp1)] - field[ijk2im(ip1, j, kp1)]
			+ field[ijk2im(ip1, jp1, k)] - field[ijk2im(ip1, j, k)]
			+ field[ijk2im(i, jp1, kp1)] - field[ijk2im(i, j, kp1)]
			+ field[ijk2im(i, jp1, k)] - field[im]
			) * INV_4DX;
		break;
	case 2:
		retval = (
			field[ijk2im(ip1, jp1, kp1)] - field[ijk2im(ip1, jp1, k)]
			+ field[ijk2im(ip1, j, kp1)] - field[ijk2im(ip1, j, k)]
			+ field[ijk2im(i, jp1, kp1)] - field[ijk2im(i, jp1, k)]
...
inline double calc_gradient_o2_to_o1(const double * field, int im, int dir) {
	...
	case 0:
		retval = (
			field[im] - field[ijk2im(im1, j, k)]
			+ field[ijk2im(i, jm1, k)] - field[ijk2im(im1, jm1, k)]
			+ field[ijk2im(i, j, km1)] - field[ijk2im(im1, j, km1)]
			+ field[ijk2im(i, jm1, km1)] - field[ijk2im(im1, jm1, km1)]
			) * INV_4DX;
		break;
	case 1:
		retval = (
			field[im] - field[ijk2im(i, jm1, k)]
			+ field[ijk2im(im1, j, k)] - field[ijk2im(im1, jm1, k)]
			+ field[ijk2im(i, j, km1)] - field[ijk2im(i, jm1, km1)]
			+ field[ijk2im(im1, j, km1)] - field[ijk2im(im1, jm1, km1)]
			) * INV_4DX;
		break;
	case 2:
		retval = (
			field[im] - field[ijk2im(i, j, km1)]
			+ field[ijk2im(im1, j, k)] - field[ijk2im(im1, j, km1)]
			+ field[ijk2im(i, jm1, k)] - field[ijk2im(i, jm1, km1)]
			+ field[ijk2im(im1, jm1, k)] - field[ijk2im(im1, jm1, km1)]
			) * INV_4DX;
		break;
	default:
	...
	for (int i = 0; i < NX; i++) {
		for (int j = 0; j < NY; j++) {
			for (int k = 0; k < NZ; k++) {
				...
				s[im] = (RHO * INV_DT) * (dux_dx + duy_dy + duz_dz)
					- RHO * (drx_dx + dry_dy + drz_dz);
			}
		}
	}
}
...
inline void Make_phi_s(double *phi, double *phi_sum, Particle *p, const double &dx, const int &np_domain, int **sekibun_cell, const int Nlattice[DIM], CTime & jikan, const double radius = RADIUS) {
#pragma omp parallel for 
...
inline void Make_phi_s_OBL(double *phi, double *phi_sum, Particle *p, const double &dx, const int &np_domain, int **sekibun_cell, const int Nlattice[DIM], const double gt, const double sreff, CTime & jikan, const double radius = RADIUS) {
#pragma omp parallel for
	for (int n = 0; n < Particle_Number; n++) {
		double xp[DIM];
		double dmyy;
...
inline double calc_gamma(double **u, int im) {
	...
	double invariant_half = 2. * (dux_dx * dux_dx + duy_dy * duy_dy + duz_dz * duz_dz)
		+ (duy_dx + dux_dy) * (duy_dx + dux_dy) + (duz_dy + duy_dz) * (duz_dy + duy_dz) + (dux_dz + duz_dx) * (dux_dz + duz_dx);

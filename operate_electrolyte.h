...
/*!
  ...
  \details Given the current particle data (positions), computes the smooth particle profile \f$\phi\f$ and the colloid
  surface charge density \f$\sigma_e\f$ \param[out] phi smooth particle profile field \param[out] surface colloid
  surface charge density \param[in] p Current particle data
 */
void Make_phi_qq_particle(double *phi, double *surface, Particle *p);

void Make_phi_qq_particle_norm(double *phi, double *surface, Particle *p);

void Make_phi_qq_fixed_particle(double *phi, double *surface, Particle *p);

/*!
  Compute the solute source term appearing in the rhs of the Navier-Stokes equation (including possible contributions
  from the external field) \details \f[
...
/*!
  ...
  \details Computes the total charge density field  by adding the colloid surface charge density (for the current
  particle positions) to the solute charge density, where the total charge density is given by \f[ \rho_e =
  (1-\phi)\sum_\alpha Z_\alpha e C_\alpha + \norm{\nabla \phi} e \sigma_e \f] with \f$\sigma_e\f$ the colloid surface
  charge density (defined over the particle/fluid interface). \param[in] p particle data \param[in] conc_k solute
  concentration field (reciprocal space) \param[out] charge_density total charge density (real space) \param[in,out] phi
  auxiliary field to compute the smooth profile \param[in,out] dmy_value auxiliary field to compute the concentration of
  a single solute species
 */
...
/*!
  ...
  \details Given the Fourier Transform of the total charge density \f$\ft{\rho}_e\f$, returns the Fourier Transform of
  the electrostatic potential by solving Poisson's Equation \f[ \ft{\Phi}(\vec{k}) = \frac{\ft{\rho}_e}{(2\pi k)^2
  \epsilon} \f] \param[in,out] potential charge density on input, electrostatic potential on output
 */

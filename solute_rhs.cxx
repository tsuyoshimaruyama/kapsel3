  ...
  for (int n = 0; n < Particle_Number; n++) {
    ...
    for (int mesh = 0; mesh < NP_domain; mesh++) {
      ...
      if (norm2 > 0.) {
        ...
        dmy = 0.;
        for (int d = 0; d < DIM; d++) {
          dmy += solute_flux_x[d][(r_mesh[0] * NY * NZ_) + (r_mesh[1] * NZ_) + r_mesh[2]] * normal[d];
        }
        dmy /= norm2;
        for (int d = 0; d < DIM; d++) {
          solute_flux_x[d][(r_mesh[0] * NY * NZ_) + (r_mesh[1] * NZ_) + r_mesh[2]] -= normal[d] * dmy;
        }
      }
    }
  }
    ...
    for (int j = 0; j < NY; j++) {
      for (int k = 0; k < NZ; k++) {
        ...
        for (int d = 0; d < DIM; d++) {
          ...
          dmy_conc_flux[d] = dmy_conc * (onsager_coeff * dmy_interaction);
        }
        for (int d = 0; d < DIM; d++) {

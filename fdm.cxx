...
void NS_solver_slavedEuler_Shear_OBL_explicit(double **u, double * Pressure, Particle *p, CTime &jikan) {
	...
	if (PHASE_SEPARATION) {
		...
		Set_poisson_rhs(u, adv, lap, rhs, jikan); // the term arising from advection of the frame is included in advection term
	}
...
void NS_solver_slavedEuler_Shear_OBL_implicit(double **& u, double * Pressure, Particle *p, CTime &jikan) {
	...
	if (PHASE_SEPARATION) {
		...
		Reset_phi(w_v3[1]);
		Reset_phi(w_v3[2]);
		Make_phi_s_OBL(w_v3[1], w_v3[2], p, DX, NP_domain, Sekibun_cell, Ns, gt, (3. * Shear_rate_eff - sreff_old) / 2., jikan);
		A2a_oblique(w_v3[1]);
	...
	if (PHASE_SEPARATION) {
		if (VISCOSITY_CHANGE) {
#ifdef _LIS_SOLVER
			CHNS_MAC_solver_implicit_viscosity_OBL(u, Pressure, u_s, stress, eta_s, jikan, degree_oblique, is_ns, ie_ns);
#else
			CHNS_MAC_solver_implicit_viscosity_OBL(u, Pressure, u_s, stress, eta_s, jikan, degree_oblique, 0, NX*NY*NZ*DIM);
#endif
		} else {
#ifdef _LIS_SOLVER
	...
	for (int i = 0; i < NX; i++) {
		for (int j = 0; j < NY; j++) {
			for (int k = 0; k < NZ; k++) {
				...
				double d2ux_dxdy = (u[0][im_ip1_jp1] - u[0][im_ip1_jm1] - u[0][im_im1_jp1] + u[0][im_im1_jm1]) * INV_4DX2;
				double d2uy_dxdy = (u[1][im_ip1_jp1] - u[1][im_ip1_jm1] - u[1][im_im1_jp1] + u[1][im_im1_jm1]) * INV_4DX2;
				double d2uz_dxdy = (u[2][im_ip1_jp1] - u[2][im_ip1_jm1] - u[2][im_im1_jp1] + u[2][im_im1_jm1]) * INV_4DX2;

				lap_u[0][im] = (1. + degree_oblique *  degree_oblique) * d2ux_dx2 - (2. *  degree_oblique * d2ux_dxdy) + d2ux_dy2 + d2ux_dz2;
				lap_u[1][im] = (1. + degree_oblique *  degree_oblique) * d2uy_dx2 - (2. *  degree_oblique * d2uy_dxdy) + d2uy_dy2 + d2uy_dz2;
				lap_u[2][im] = (1. + degree_oblique *  degree_oblique) * d2uz_dx2 - (2. *  degree_oblique * d2uz_dxdy) + d2uz_dy2 + d2uz_dz2;
			}
		}
	}
}

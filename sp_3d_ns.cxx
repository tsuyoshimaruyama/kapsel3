  ...
  if ((SW_EQ == Shear_Navier_Stokes) || (SW_EQ == Shear_Navier_Stokes_Lees_Edwards) ||
	...
	for (jikan.ts = resumed_ts; jikan.ts <= MSTEP; jikan.ts++) {
		...
		if (jikan.ts % GTS == 0) {
          ...
          Shear_Navier_Stokes_Lees_Edwards) { Cpy_v3(work_v3, u); U_k2u(work_v3); calc_shear_rate_field(work_v3,
          shear_rate_field); } else if (SW_EQ == Navier_Stokes_Cahn_Hilliard_FDM || SW_EQ ==
          Shear_Navier_Stokes_Lees_Edwards_FDM || SW_EQ == Shear_NS_LE_CH_FDM) { calc_shear_rate_field(u,
          shear_rate_field);
						}

...
/*!
  ...
  \f{eqnarray*}{
  |\nabla\phi(x)| &=&
  \frac{h'(a + \zeta/2 -x)h(x - a + \zeta/2) + h'(x - a + \zeta/2) h(a + \zeta/2 -r)}{\left[h(a + \zeta/2 - x) + h(x - a
  + \zeta/2)\right]^2} \\
...
/*!
  ...
  \details sekibun_cell is a list of np_domain number of grid points (i,j,k) in local coordinates (i.e with respect to
  the center of the grid particle) that should be considered when performing integrals over the particle domain. \note
  The center of the sekibun_cell need not coincide with the actual particle center, which is why a distance larger than
  the particle radius is used when deciding if a point is in/out the particle domain. \param[in] profile_func profile
  function used to determine interface/fluid boundary \param[out] np_domain number of points in the sekibun_cell list

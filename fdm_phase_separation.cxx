	...
	for (int i = 0; i < NX; i++) {
		for (int j = 0; j < NY; j++) {
			for (int k = 0; k < NZ; k++) {
				...
				cp[im] = potential_deriv(psi[im]) - ps.alpha * lap_psi + ps.w * A_XI * grad_phi_norm + 2. * ps.d * (psi[im] - ps.neutral) * phi[im];
			}
		}
	}
}
	...
	for (int i = 0; i < NX; i++) {
		for (int j = 0; j < NY; j++) {
			for (int k = 0; k < NZ; k++) {
				...
				double dphi_dx_contra = ((1. + degree_oblique * degree_oblique) * dphi_dx_co) - (degree_oblique * dphi_dy_co);
...
void Set_poisson_rhs_viscosity(double **u, double **u_s, double ** adv, double ** lap, double **stress, double *eta_s, double *s, CTime & jikan) {
#pragma omp parallel for
	for (int i = 0; i < NX; i++) {
		for (int j = 0; j < NY; j++) {
			for (int k = 0; k < NZ; k++) {
				...
				for (int d = 0; d < DIM; d ++) {
					...
					case 0:
						vt = 2. * calc_gradient_o1_to_o1(eta_s, im, 0) * calc_gradient_o1_to_o1(u_s[0], im, 0)
							+ calc_gradient_o1_to_o1(eta_s, im, 1) * (calc_gradient_o1_to_o1(u_s[0], im, 1) + calc_gradient_o1_to_o1(u_s[1], im, 0))
							+ calc_gradient_o1_to_o1(eta_s, im, 2) * (calc_gradient_o1_to_o1(u_s[0], im, 2) + calc_gradient_o1_to_o1(u_s[2], im, 0));
						break;
					case 1:
						vt = 2. * calc_gradient_o1_to_o1(eta_s, im, 1) * calc_gradient_o1_to_o1(u_s[1], im, 1)
							+ calc_gradient_o1_to_o1(eta_s, im, 0) * (calc_gradient_o1_to_o1(u_s[0], im, 1) + calc_gradient_o1_to_o1(u_s[1], im, 0))
							+ calc_gradient_o1_to_o1(eta_s, im, 2) * (calc_gradient_o1_to_o1(u_s[1], im, 2) + calc_gradient_o1_to_o1(u_s[2], im, 1));
						break;
					case 2:
						vt = 2. * calc_gradient_o1_to_o1(eta_s, im, 2) * calc_gradient_o1_to_o1(u_s[2], im, 2)
							+ calc_gradient_o1_to_o1(eta_s, im, 0) * (calc_gradient_o1_to_o1(u_s[0], im, 2) + calc_gradient_o1_to_o1(u_s[2], im, 0))
							+ calc_gradient_o1_to_o1(eta_s, im, 1) * (calc_gradient_o1_to_o1(u_s[1], im, 2) + calc_gradient_o1_to_o1(u_s[2], im, 1));
					}
					vt *= IRHO;
...
void Set_poisson_rhs_viscosity_OBL(double **u, double **u_s, double ** adv, double ** lap, double **stress, double *eta_s, double *s, CTime & jikan) {
	...
	for (int i = 0; i < NX; i++) {
		for (int j = 0; j < NY; j++) {
			for (int k = 0; k < NZ; k++) {
				...
				double advective_term = calc_gradient_o1_to_o1(w_v3_3[0], im, 0) + calc_gradient_o1_to_o1(w_v3_3[1], im, 1) + calc_gradient_o1_to_o1(w_v3_3[2], im, 2);
...
void Output_xdmf_sca(std::string filename, std::string hdffilename, std::string dataname, CTime & jikan) {
	...
	for (int i = istart; i <= MSTEP; i++) {
		if (i % GTS == 0) {
			...
			os_out << R"(<Topology Name="tp" TopologyType="3DCORECTMesh" NumberOfElements=")" << NZ << " " << NY << " " << NX << R"("/>)" << std::endl;
			os_out << R"(<Geometry Name="geo" GeometryType="ORIGIN_DXDYDZ">)" << std::endl;
			os_out << R"(<DataItem Name="Origin" Dimensions="3" NumberType="Float" Precision="4" Format="XML">)" << std::endl;
			os_out << "0 0 0" << std::endl;
			os_out << "</DataItem>" << std::endl;
			os_out << R"(<DataItem Name="Spacing" Dimensions="3" NumberType="Float" Precision="4" Format="XML">)" << std::endl;
			os_out << DX << " " << DX << " " << DX << std::endl;
			os_out << "</DataItem>" << std::endl;
			os_out << "</Geometry>" << std::endl;
			os_out << R"(<Attribute Name = ")" << filename << R"(" AttributeType = "Scalar" Center = "Node">)" << std::endl;
			os_out << R"(<DataItem Dimensions=")" << NZ << " " << NY << " " << NX << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
...
void Output_xdmf_vec(std::string filename, std::string hdffilename, std::string dataname_0, std::string dataname_1, std::string dataname_2, CTime & jikan) {
	std::stringstream ss_filename;
	ss_filename << filename << ".xmf";
	std::ofstream os_out(ss_filename.str().c_str(), std::ios::out);
	...
	for (int i = istart; i <= MSTEP; i++) {
		if (i % GTS == 0) {
			...
			os_out << R"(<Attribute Name = ")" << filename << R"(" AttributeType = "Vector" Center = "Node">)" << std::endl;
			os_out << R"(<DataItem ItemType="Function" Dimensions=")" << NZ << " " << NY << " " << NX << R"( 3" Function = "JOIN($0, $1, $2) ">)" << std::endl;
...
void Output_xdmf_particle(std::string filename, std::string hdffilename, CTime & jikan) {
	...
	for (int i = istart; i <= MSTEP; i++) {
		if (i % GTS == 0) {
			...
			os_out << R"(<Topology Name="tp" TopologyType="Polyvertex" NumberOfElements=")" << Particle_Number << R"("/>)" << std::endl;
			//os_out << R"(<Geometry Name="geo" GeometryType="ORIGIN_DXDYDZ">)" << std::endl;
			//os_out << R"(<DataItem Name="Origin" Dimensions="3" NumberType="Float" Precision="4" Format="XML">)" << std::endl;
			//os_out << "0 0 0" << std::endl;
			//os_out << "</DataItem>" << std::endl;
			...
			//os_out << R"(<Attribute Name = ")" << filename << R"(" AttributeType = "Vector" Center = "Node">)" << std::endl;
			os_out << R"(<DataItem ItemType="Function" Dimensions=")" << Particle_Number << R"( 3" Function = "JOIN($0, $1, $2) ">)" << std::endl;

			os_out << R"(<DataItem Dimensions=")" << Particle_Number << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "RX" << std::endl;
			os_out << "</DataItem>" << std::endl;
			os_out << R"(<DataItem Dimensions=")" << Particle_Number << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "RY" << std::endl;
			os_out << "</DataItem>" << std::endl;
			os_out << R"(<DataItem Dimensions=")" << Particle_Number << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "RZ" << std::endl;
			os_out << "</DataItem>" << std::endl;
			...
			os_out << R"(<DataItem Dimensions=")" << Particle_Number << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "VX" << std::endl;
			os_out << "</DataItem>" << std::endl;
			os_out << R"(<DataItem Dimensions=")" << Particle_Number << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "VY" << std::endl;
			os_out << "</DataItem>" << std::endl;
			os_out << R"(<DataItem Dimensions=")" << Particle_Number << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "VZ" << std::endl;
			os_out << "</DataItem>" << std::endl;
...
void Output_xdmf_particle_single(std::string filename, std::string hdffilename, CTime & jikan) {
	...
	for (int i = istart; i <= MSTEP; i++) {
		if (i % GTS == 0) {
			...
			//os_out << R"(<Attribute Name = ")" << filename << R"(" AttributeType = "Vector" Center = "Node">)" << std::endl;
			os_out << R"(<DataItem ItemType="Function" Dimensions=")" << Particle_Number + 1 << R"( 3" Function = "JOIN($0, $1, $2) ">)" << std::endl;

			os_out << R"(<DataItem Dimensions=")" << Particle_Number + 1 << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "RX" << std::endl;
			os_out << "</DataItem>" << std::endl;
			os_out << R"(<DataItem Dimensions=")" << Particle_Number + 1 << R"(" NumberType="Float" Precision="8" Format="HDF">)" << std::endl;
			os_out << hdffilename << "_" << i / GTS << ".h5:/" << "RY" << std::endl;
...
int Output_hdf5_vec(std::string filename, std::string dataname_0, std::string dataname_1, std::string dataname_2, double **data_3d, int count) {
	std::stringstream ss_filename;
	ss_filename << filename << "_" << count << ".h5";
...
void xdmf_output(CTime jikan) {
	...
	if (SW_EQ == Navier_Stokes_Cahn_Hilliard_FDM || SW_EQ == Shear_Navier_Stokes_Lees_Edwards_FDM || SW_EQ == Shear_NS_LE_CH_FDM) {

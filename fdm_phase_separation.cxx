        ...
        for (int d = 0; d < DIM; d++) {
          ...
          switch (d) {
            case 0:
              ...
              break;
            case 1:
              g22 *eta_x *u_xy + (g12 * eta_x + 2. * g22 * eta_y) * u_yy + g22 *eta_z *u_zy + g33 *eta_z *u_yz;
              break;
            case 2:
  ...
  for (int i = 0; i < NX; i++) {
    for (int j = 0; j < NY; j++) {
      for (int k = 0; k < NZ; k++) {
        ...
        double advective_term = calc_gradient_o1_to_o1(w_v3_3[0], im, 0) + calc_gradient_o1_to_o1(w_v3_3[1], im, 1) +
...
void Output_xdmf_sca(std::string filename, std::string hdffilename, std::string dataname, CTime &jikan) {
  ...
  for (int i = istart; i <= MSTEP; i++) {
    if (i % GTS == 0) {
      ...
      os_out << R"(<Topology Name="tp" TopologyType="3DCORECTMesh" NumberOfElements=")" << NZ << " " << NY << " " << NX
             << R"("/>)" << std::endl;
...
void Output_xdmf_particle(std::string filename, std::string hdffilename, CTime &jikan) {
  ...
  for (int i = istart; i <= MSTEP; i++) {
    if (i % GTS == 0) {
      ...
      os_out << R"(<Topology Name="tp" TopologyType="Polyvertex" NumberOfElements=")" << Particle_Number << R"("/>)"
             << std::endl;
      // os_out << R"(<Geometry Name="geo" GeometryType="ORIGIN_DXDYDZ">)" << std::endl;
      ...
      // << DX << " " << DX << std::endl; os_out << "</DataItem>" << std::endl; os_out << "</Geometry>" << std::endl;
      ...
      os_out << R"(<DataItem Dimensions=")" << Particle_Number << R"(" NumberType="Float" Precision="8" Format="HDF">)"
             << std::endl;

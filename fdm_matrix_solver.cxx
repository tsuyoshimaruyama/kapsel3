...
#pragma omp parallel for
	for (int idx = is_ns; idx < ie_ns; idx++) {
		...
		double adv_term = -0.5 * (
			+ u_s[0][im] * (u[d][ijk2im(ip1, j, k)] - u[d][ijk2im(im1, j, k)]) / _2DX
		...
		switch (d) {
		case 0:
			ps = pressure[im] - pressure[ijk2im(im1, j, k)]
				+ pressure[ijk2im(i, jm1, k)] - pressure[ijk2im(im1, jm1, k)]
				+ pressure[ijk2im(i, j, km1)] - pressure[ijk2im(im1, j, km1)]
				+ pressure[ijk2im(i, jm1, km1)] - pressure[ijk2im(im1, jm1, km1)];
			break;
		case 1:
			ps = pressure[im] - pressure[ijk2im(i, jm1, k)]
				+ pressure[ijk2im(im1, j, k)] - pressure[ijk2im(im1, jm1, k)]
				+ pressure[ijk2im(i, j, km1)] - pressure[ijk2im(i, jm1, km1)]
				+ pressure[ijk2im(im1, j, km1)] - pressure[ijk2im(im1, jm1, km1)];
			break;
		case 2:
			ps = pressure[im] - pressure[ijk2im(i, j, km1)]
				+ pressure[ijk2im(im1, j, k)] - pressure[ijk2im(im1, j, km1)]
				+ pressure[ijk2im(i, jm1, k)] - pressure[ijk2im(i, jm1, km1)]
...
void CHNS_MAC_solver_implicit(double **u, double * pressure, double **u_s, double **stress_s, const CTime &jikan, int is_ns, int ie_ns) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
void CHNS_MAC_solver_implicit_viscosity(double **u, double * pressure, double **u_s, double **stress_s, double *eta_s, const CTime &jikan, int is_ns, int ie_ns) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
	...
	for (int idx = is_ns; idx < ie_ns; idx++) {
		...
		case 0:
			vt = calc_gradient_o1_to_o1(eta_s, im, 0) * calc_gradient_o1_to_o1(u[0], im, 0)
				+ 0.5 * calc_gradient_o1_to_o1(eta_s, im, 1) * (calc_gradient_o1_to_o1(u[0], im, 1) + calc_gradient_o1_to_o1(u[1], im, 0))
				+ 0.5 * calc_gradient_o1_to_o1(eta_s, im, 2) * (calc_gradient_o1_to_o1(u[0], im, 2) + calc_gradient_o1_to_o1(u[2], im, 0));
			break;
		case 1:
			vt = calc_gradient_o1_to_o1(eta_s, im, 1) * calc_gradient_o1_to_o1(u[1], im, 1)
				+ 0.5 * calc_gradient_o1_to_o1(eta_s, im, 0) * (calc_gradient_o1_to_o1(u[0], im, 1) + calc_gradient_o1_to_o1(u[1], im, 0))
				+ 0.5 * calc_gradient_o1_to_o1(eta_s, im, 2) * (calc_gradient_o1_to_o1(u[1], im, 2) + calc_gradient_o1_to_o1(u[2], im, 1));
			break;
		case 2:
			vt = calc_gradient_o1_to_o1(eta_s, im, 2) * calc_gradient_o1_to_o1(u[2], im, 2)
				+ 0.5 * calc_gradient_o1_to_o1(eta_s, im, 0) * (calc_gradient_o1_to_o1(u[0], im, 2) + calc_gradient_o1_to_o1(u[2], im, 0))
				+ 0.5 * calc_gradient_o1_to_o1(eta_s, im, 1) * (calc_gradient_o1_to_o1(u[1], im, 2) + calc_gradient_o1_to_o1(u[2], im, 1));
			break;
		}
		vt *= IRHO;
...
void NS_MAC_solver_implicit_OBL(double **u, double * pressure, double **u_s, const CTime &jikan, const double degree_oblique, int is_ns, int ie_ns) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
void CHNS_MAC_solver_implicit_OBL(double **u, double * pressure, double **u_s, double **stress_s, const CTime &jikan, const double degree_oblique, int is_ns, int ie_ns) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
#pragma omp parallel for
	for (int idx = is_ns; idx < ie_ns; idx++) {
		...
		double stress_term = - stress[d][im];
		double adv_frame_term = (d == 0) ? - Shear_rate_eff * u[1][im] : 0.;
		double bs = u[d][im] / jikan.dt_fluid + adv_term + pressure_term + viscosity_term + adv_frame_term + stress_term;
...
void CHNS_MAC_solver_implicit_viscosity_OBL(double **u, double * pressure, double **u_s, double **stress_s, double *eta_s, const CTime &jikan, const double degree_oblique, int is_ns, int ie_ns) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
void CH_solver_implicit_euler(double * psi, double * psi_o, double * phi, double ** u, const CTime &jikan, int is_ch, int ie_ch) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
#pragma omp parallel for
		for (int idx = is_ch; idx < ie_ch; idx++) {
			...
			val_ch_csr[idx2 + 19] = u[0][ijk2im(ip1, j, k)] * INV_2DX - 12. * ak - 2. * kiDX2 * ps.d * phi[ijk2im(ip1, j, k)];

			idx_ch_csr[idx2 + 20] = ijk2idx(im1, j, k);
			val_ch_csr[idx2 + 20] = -u[0][ijk2im(im1, j, k)] * INV_2DX - 12. * ak - 2. * kiDX2 * ps.d * phi[ijk2im(im1, j, k)];

			//-----------------

			idx_ch_csr[idx2 + 21] = ijk2idx(i, jp1, k);
			val_ch_csr[idx2 + 21] = u[1][ijk2im(i, jp1, k)] * INV_2DX - 12. * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, jp1, k)];

			idx_ch_csr[idx2 + 22] = ijk2idx(i, jm1, k);
			val_ch_csr[idx2 + 22] = -u[1][ijk2im(i, jm1, k)] * INV_2DX - 12. * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, jm1, k)];

			//-----------------

			idx_ch_csr[idx2 + 23] = ijk2idx(i, j, kp1);
			val_ch_csr[idx2 + 23] = u[2][ijk2im(i, j, kp1)] * INV_2DX - 12. * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, j, kp1)];

			idx_ch_csr[idx2 + 24] = ijk2idx(i, j, km1);
			val_ch_csr[idx2 + 24] = -u[2][ijk2im(i, j, km1)] * INV_2DX - 12. * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, j, km1)];
...
#pragma omp parallel for
	for (int idx = is_ch; idx < ie_ch; idx++) {
		...
		double bs = psi_im * INV_DT
			+ kiDX2 * (
				  potential_deriv(psi[ijk2im(ip1, j, k)])
				+ potential_deriv(psi[ijk2im(im1, j, k)])
				+ potential_deriv(psi[ijk2im(i, jp1, k)])
				+ potential_deriv(psi[ijk2im(i, jm1, k)])
				+ potential_deriv(psi[ijk2im(i, j, kp1)])
				+ potential_deriv(psi[ijk2im(i, j, km1)])
				- 6. * potential_deriv(psi_im))
			+ ps.w * A_XI * kiDX2 * (
				  calc_gradient_norm(phi, ijk2im(ip1, j, k))
				+ calc_gradient_norm(phi, ijk2im(im1, j, k))
				+ calc_gradient_norm(phi, ijk2im(i, jp1, k))
				+ calc_gradient_norm(phi, ijk2im(i, jm1, k))
				+ calc_gradient_norm(phi, ijk2im(i, j, kp1))
				+ calc_gradient_norm(phi, ijk2im(i, j, km1))
...
void CH_solver_implicit_bdfab(double * psi, double * psi_o, double * phi, double ** u, const CTime &jikan, int is_ch, int ie_ch) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
#pragma omp parallel for
	for (int idx = is_ch; idx < ie_ch; idx++) {
		...
		double bs = 0.5 * (4. * psi_im - psi_o_im) * INV_DT
			+ 2. * kiDX2 * (
				potential_deriv(psi[ijk2im(ip1, j, k)])
				+ potential_deriv(psi[ijk2im(im1, j, k)])
				+ potential_deriv(psi[ijk2im(i, jp1, k)])
				...
				- 6. * potential_deriv(psi_im))
			- kiDX2 * (
				potential_deriv(psi_o[ijk2im(ip1, j, k)])
				+ potential_deriv(psi_o[ijk2im(im1, j, k)])
				+ potential_deriv(psi_o[ijk2im(i, jp1, k)])
				+ potential_deriv(psi_o[ijk2im(i, jm1, k)])
				+ potential_deriv(psi_o[ijk2im(i, j, kp1)])
				+ potential_deriv(psi_o[ijk2im(i, j, km1)])
...
void CH_solver_implicit_euler_OBL(double * psi, double * psi_o, double * phi, double ** u, const CTime &jikan, const double degree_oblique, int is_ch, int ie_ch) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
#pragma omp parallel for
		for (int idx = is_ch; idx < ie_ch; idx++) {
			...
			idx_ch_csr[idx2 + 0] = ijk2idx(i, j, k);
			val_ch_csr[idx2 + 0] = INV_DT + (42. + 29. * gt2 + 6. * gt4) * ak + 4. * (3. + gt2) * kiDX2 * ps.d * phi[im];
			...
			idx_ch_csr[idx2 + 19] = ijk2idx(ip1, j, k);
			val_ch_csr[idx2 + 19] = u[0][ijk2im(ip1, j, k)] * INV_2DX - 4. * (3. + 4. * gt2 + gt4) * ak - (1. + gt2) * 2. * kiDX2 * ps.d * phi[ijk2im(ip1, j, k)];

			idx_ch_csr[idx2 + 20] = ijk2idx(im1, j, k);
			val_ch_csr[idx2 + 20] = -u[0][ijk2im(im1, j, k)] * INV_2DX - 4. * (3. + 4. * gt2 + gt4) * ak - (1. + gt2) * 2. * kiDX2 * ps.d * phi[ijk2im(im1, j, k)];

			//-----------------

			idx_ch_csr[idx2 + 21] = ijk2idx(i, jp1, k);
			val_ch_csr[idx2 + 21] = u[1][ijk2im(i, jp1, k)] * INV_2DX - 4. * (3. + gt2) * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, jp1, k)];

			idx_ch_csr[idx2 + 22] = ijk2idx(i, jm1, k);
			val_ch_csr[idx2 + 22] = -u[1][ijk2im(i, jm1, k)] * INV_2DX - 4. * (3. + gt2) * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, jm1, k)];

			//-----------------

			idx_ch_csr[idx2 + 23] = ijk2idx(i, j, kp1);
			val_ch_csr[idx2 + 23] = u[2][ijk2im(i, j, kp1)] * INV_2DX - 4. * (3. + gt2) * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, j, kp1)];

			idx_ch_csr[idx2 + 24] = ijk2idx(i, j, km1);
			val_ch_csr[idx2 + 24] = -u[2][ijk2im(i, j, km1)] * INV_2DX - 4. * (3. + gt2) * ak - 2. * kiDX2 * ps.d * phi[ijk2im(i, j, km1)];
	...
	for (int idx = is_ch; idx < ie_ch; idx++) {
		...
		double bs = psi_im * INV_DT
			...
			(1. + gt2) * (potential_deriv(psi[ijk2im(ip1, j, k)]) + potential_deriv(psi[ijk2im(im1, j, k)]))
				...
				+ potential_deriv(psi[ijk2im(i, j, kp1)])
				+ potential_deriv(psi[ijk2im(i, j, km1)])
				- 0.5 * gt * (potential_deriv(psi[ijk2im(ip1, jp1, k)])
					- potential_deriv(psi[ijk2im(ip1, jm1, k)])
					- potential_deriv(psi[ijk2im(im1, jp1, k)])
					+ potential_deriv(psi[ijk2im(im1, jm1, k)]))
				- 2. * (3. + gt2) * potential_deriv(psi_im))
			+ ps.w * A_XI * kiDX2 * (
			(1. + gt2) * (calc_gradient_norm_OBL(phi, ijk2im(ip1, j, k), gt) + calc_gradient_norm_OBL(phi, ijk2im(im1, j, k), gt))
				...
				- 0.5 * gt * (calc_gradient_norm_OBL(phi, ijk2im(ip1, jp1, k), gt)
			...
			- 2. * ps.neutral * ps.d * kiDX2 *  (
			(1. + gt2) * (phi[ijk2im(ip1, j, k)] + phi[ijk2im(im1, j, k)])
				+ phi[ijk2im(i, jp1, k)]
				+ phi[ijk2im(i, jm1, k)]
				+ phi[ijk2im(i, j, kp1)]
				+ phi[ijk2im(i, j, km1)]
				- 0.5 * gt * (phi[ijk2im(ip1, jp1, k)] - phi[ijk2im(ip1, jm1, k)] - phi[ijk2im(im1, jp1, k)] + phi[ijk2im(im1, jm1, k)])
...
void CH_solver_implicit_bdfab_OBL(double * psi, double * psi_o, double * phi, double ** u, const CTime &jikan, const double degree_oblique, int is_ch, int ie_ch) {
#ifdef _LIS_SOLVER
	LIS_INT iter;
#endif
	const double DX2 = DX * DX;
...
#pragma omp parallel for
		for (int idx = is_ch; idx < ie_ch; idx++) {
			...
			idx_ch_csr[idx2 + 0] = ijk2idx(i, j, k);
			val_ch_csr[idx2 + 0] = 1.5 * INV_DT + (42. + 29. * gt2 + 6. * gt4) * ak + 4. * (3. + gt2) * kiDX2 * ps.d * phi[im];
	...
	for (int idx = is_ch; idx < ie_ch; idx++) {
		...
		double bs = 0.5 * (4. * psi_im - psi_o_im) * INV_DT
			...
			(1. + gt2) * (potential_deriv(psi_o[ijk2im(ip1, j, k)]) + potential_deriv(psi_o[ijk2im(im1, j, k)]))
				...
				+ potential_deriv(psi_o[ijk2im(i, j, kp1)])
				+ potential_deriv(psi_o[ijk2im(i, j, km1)])
				- 0.5 * gt * (potential_deriv(psi_o[ijk2im(ip1, jp1, k)])
					- potential_deriv(psi_o[ijk2im(ip1, jm1, k)])
					- potential_deriv(psi_o[ijk2im(im1, jp1, k)])
					+ potential_deriv(psi_o[ijk2im(im1, jm1, k)]))
...
void Init_lis(int argc, char *argv[]) {
	...
	lis_solver_set_option("-i 4 -p 0 -initx_zeros 1 -tol 1.0e-9 -maxiter 1000 -f 0 -print 0", lis_solver_ns);  // -i 4 BiCGSTAB, 6 GPBiCG, 9 GMRES see Lis Documents  

...
inline void Copy_sekibun_cell(const int np_domain, int**& src, int**& dest) {
  {
...
bool sekibun_comparison(int* a, int* b) { return SQ(a[0]) + SQ(a[1]) + SQ(a[2]) < SQ(b[0]) + SQ(b[1]) + SQ(b[2]); }
  ...
  for (int i = -nh; i <= nh; i++) {
    for (int j = -nh; j <= nh; j++) {
      for (int k = -nh; k <= nh; k++) {
        ...
        double dmy = sqrt(SQ(dmy_x[0] - dmy_xp[0]) + SQ(dmy_x[1] - dmy_xp[1]) + SQ(dmy_x[2] - dmy_xp[2]));

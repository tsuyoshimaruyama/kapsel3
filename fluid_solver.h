...
/*!
  \fn void Field_solver_Euler(const int &dim, double **zeta_k, const CTime &jikan, double **rhs, const Index_range
  &ijk_range) \brief solver euler
 */

/*!
  \fn void Rhs_NS(double **zeta, double uk_dc[DIM], double **rhs, const Index_range *ijk_range, const int &n_ijk_range,
  Particle *p) \brief rhs of ns
 */
...
/*!
  \fn void Rhs_NS_solute(Particle *p, double **zeta_k, double uk_dc[DIM], double **u, double **concentration_k, double
  **rhs_ns, double **rhs_solute, const Index_range *ijk_range, const int &n_ijk_range, double **solute_flux, double
  **grad_potential, double **surface_normal, double rhs_uk_dc[DIM]) \brief rhs + ns
 */
...
/*!
  \fn Rhs_NS_Nernst_Planck(Particle *p, double **zeta, double uk_dc[DIM], double **u, double **concentration_k, double
  **rhs_ns, double **rhs_solute, const Index_range *ijk_range, const int &n_ijk_range, double **solute_flux, double
  **grad_potential, double **surface_normal, double rhs_uk_dc[DIM], const CTime &jikan) \brief ns + np
 */
...
/*!
  ...
  \details \f[
  \ft{\vec{\zeta}} \longrightarrow \ft{\vec{\zeta}} + \left(e^{-\nu (2\pi k)^2 h} - 1\right)\left[\ft{\vec{\zeta}} +
  \frac{\ft{\vec{\Omega}}^*}{\nu(2\pi k)^2}\right] \f] Analytic solution is valid in the absence of solute terms, with
  no shear. \param[in,out] zeta reduced vorticity field \param[in] jikan time data \param[in] uk_dc zero-wavenumber
  Fourier transform of the velocity field \param[in] ijk_range field iterator parameters for update \param[in]
  n_ijk_range field iterator parameters for update \param[in] p particle data (unused) \see \ref page_design_fsolver
  section of manual for further details. \todo Specify the difference between ijk_range and n_ijk_range
 */
void NS_solver_slavedEuler(double **          zeta,
                           const CTime &      jikan,
                           double             uk_dc[DIM],
...
void Ion_diffusion_solver_Euler(double **          zeta,
                                ...
                                const int &        n_ijk_range);
/*!
  \brief Solves Navier-Stokes and Poisson-Nernst Plank equation to update the fluid velocity and the electrolyte
  concentration \details Calls Rhs_NS_Nernst_Plank to compute the various terms appearing in the right hand side of the
  relevant equations, then proceeds to call Field_solver_Euler to update the reduced vorticity field \f$\vec{\zeta}\f$
  and the solute concentration \f$C_{\alpha}\f$. \param[in,out] zeta reduced vorticity field (reciprocal space)
  \param[in] jikan time data
  ...
  \note The solute source term appearing in the rhs of the Navier-Stokes equation has not been included. This section of
  the code should probably be removed for clarity... \see \ref page_design_ssolver section of manual for furter details.
 */
    ...
    for (int i = 0; i < NX; i++) {
        for (int j = 0; j < NY; j++) {
            for (int k = 0; k < NZ_; k++) {
                ...
                K2[im] = SQ(WAVE_X * KX_int[im]) + SQ(WAVE_Y * KY_int[im] - WAVE_X * KX_int[im] * degree_oblique) +
                         SQ(WAVE_Z * KZ_int[im]);

...
#pragma omp parallel for default(none) shared(Update_Particle_Number, p)
    for(int n = 0; n < Update_Particle_Number; n++){
        for(int d = 0; d < DIM; d++){
            ...
            p[n].torque_r_previous[d] = p[n].torque_r[d];
            p[n].torque_r[d] = 0.0;
...
void MD_solver_position_Euler(Particle *p, const CTime &jikan){
    if(SW_PT != rigid){
        double delta_x;
#pragma omp parallel for default(none) shared(Update_Particle_Number, p, jikan, L_particle, L, janus_propulsion, ROTATION) private(delta_x)
        for(int n = 0; n < Update_Particle_Number; n++) {
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared (Update_Particle_Number, p, jikan, IMASS, IMOI, janus_propulsion) private(dmy, dmy_rot, self_force, self_torque)
#endif
        for(int n = 0; n < Update_Particle_Number; n++){
            ...
            if(janus_propulsion[p[n].spec] != obstacle){
                ...
                for(int d = 0; d < DIM; d++) {
                    ...
                    p[n].omega[d] += ( dmy_rot * (p[n].torque_hydro[d] + p[n].torque_r[d] + self_torque[d]));
                }
            }else{
                for(int d = 0; d < DIM; d++){
                    p[n].v_old[d] = p[n].v[d];
    ...
    for(int n = 0; n < Update_Particle_Number; n++){
        ...
        if(janus_propulsion[p[n].spec] != obstacle){
            ...
            for(int d = 0; d < DIM; d++){
                ...
                p[n].omega_old[d] = p[n].omega[d];
                p[n].omega[d] += (dmy_rot * (p[n].torque_hydro[d] + p[n].torque_slip[d] + p[n].torque_r[d] + self_torque[d]));
            }
        }else{
            for(int d = 0; d < DIM; d++){
                p[n].v_old[d] = p[n].v[d];
...
#pragma omp parallel for default(none) shared (Update_Particle_Number, p, jikan, IMASS, IMOI, janus_propulsion) private(dmy, dmy_rot, self_force, self_torque)
    ...
    for(int n = 0; n < Update_Particle_Number; n++){
        ...
        if(janus_propulsion[p[n].spec] != obstacle){
            ...
            for(int d = 0; d < DIM; d++){
                p[n].v_old[d] = p[n].v[d];
                p[n].v[d] += dmy * (2.0 * (p[n].f_hydro[d] + p[n].f_slip[d] + self_force[d]) + p[n].fr[d] + p[n].fr_previous[d]);

                p[n].omega_old[d] = p[n].omega[d];
                p[n].omega[d] += dmy_rot * (2.0 * (p[n].torque_hydro[d] + p[n].torque_slip[d] + self_torque[d]) + p[n].torque_r[d] + p[n].torque_r_previous[d]);
            }
        }else{
            for(int d = 0; d < DIM; d++){
                p[n].v_old[d] = p[n].v[d];
...
void MD_solver_velocity_slip_iter(Particle *p, const CTime &jikan, const ITER &iter_flag){
    if(iter_flag != start_iter && iter_flag != new_iter && iter_flag != end_iter && iter_flag != reset_iter){
        ...
        for(int n = 0; n < Update_Particle_Number; n++) {
            ...
            if(janus_propulsion[p[n].spec] != obstacle){
                ...
                for(int d = 0; d < DIM; d++){
                    ...
                    p[n].v[d] += dmy * ( 2.0 * (p[n].f_hydro[d] + self_force[d]) + p[n].fr[d] + p[n].fr_previous[d]); // CN
                    p[n].omega[d] += dmy_rot * (2.0 *(p[n].torque_hydro[d] + self_torque[d]) + p[n].torque_r[d] + p[n].torque_r_previous[d]);
                }
            }else{
                for(int d = 0; d < DIM; d++){
                    p[n].v_old[d] = p[n].v[d];
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared (Update_Particle_Number, p, jikan, janus_propulsion, ROTATION) private(delta_x, delta_vx)
#endif
        for(int n = 0; n < Update_Particle_Number; n++) {
            if(janus_propulsion[p[n].spec] != obstacle){
                for(int d = 0; d < DIM; d++) {

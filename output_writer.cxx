...
const char* hdf5_writer::f_tau_name[]            = {"tau_xx", "tau_xy", "tau_xz", "tau_yy", "tau_yz", "tau_zz", "tau"};
...
template <hsize_t, hsize_t>
hsize_t* alloc_1d(hsize_t n1);
template <hsize_t>
void free_1d(hsize_t* i);
...
inline void hdf5_writer::write_particle_vectorn(const double* data,
                                                const int&    dim,
                                                const char*   name,
                                                hid_t         _loc = hid_null) {
  hid_t   loc         = (_loc == hid_null ? gid_part : _loc);
...
inline void hdf5_writer::write_obstacle_vectorn(const double* data,
                                                const int&    dim,
                                                const char*   name,
                                                hid_t         _loc = hid_null) {
  hid_t   loc         = (_loc == hid_null ? gid_pobs : _loc);
...
void hdf5_writer::write_charge_field_data(double** u,
                                          double*  phi,
...
void hdf5_writer::show_parameter() {
  ...
  if (!print_field.none) {
    for (int d = 0; d < DIM; d++) {
      fprintf(stderr,
              "# %s-axis slicing       : start = %4d, stride = %4d, count = %4d\n",
              f_axis_name[d],
              crop_field.start[d],
              crop_field.stride[d],
              crop_field.count[d]);
...
void hdf5_writer::write_configure_file() {
  ...
  if (!print_field.none) {
    ...
    if (print_field.phi) fprintf(conf, "[Field Scalar]\n %s\n", f_phi_name);
    if (print_field.pressure) fprintf(conf, "[Field Scalar]\n %s\n", f_pressure_name);
    if (print_field.tau) {
...
void hdf5_writer::write_field_info() {
  if (print_field.none) return;
  double* work_v3[DIM];
  for (int d = 0; d < DIM; d++) work_v3[d] = alloc_1d_double(NX * NY * NZ_);
  ...
  hid_t gid_dmy = H5Gcreate(gid_sys_data, gid_field_name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  h5_check_err(status);
  for (int d = 0; d < DIM; d++) free_1d_double(work_v3[d]);
}
...
#pragma omp parallel for private(i)
  ...
  hid_t gid_dmy = H5Gcreate(gid_sys_data, gid_part_name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  h5_check_err(gid_dmy);
...
#pragma omp parallel for private(i, jj)
  ...
  hid_t gid_dmy = H5Gcreate(gid_sys_data, gid_pobs_name, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  h5_check_err(gid_dmy);

/*!
  ...
  (header file) \author Y. Nakayama \date 2006/06/27 \version 1.1
 */
#ifndef SOLUTE_RHS_H
#define SOLUTE_RHS_H
...
/*!
  \brief Add the non-linear diffusive flux (due to the total charge distribution) to the rhs of the solute
  advection-diffusion equation for a given species \details \f[
  ...
  \param[in,out] solute_flux updated solute flux, including the non-linear diffusive current due to electrostatic
  interactions \param[in] valency_e valence charges of the given solute species \param[in] Onsager_coeff Onsager
  coefficient of the given solute species
 */
...
/*!
  \brief Adds the advection flux contribution (due to solvent motion) to the rhs of the advection-diffusion equation for
  a given solute species \details \f[
  -\vec{j}_\alpha(\vec{r}) \longrightarrow
...
/*!
  \brief Returns the total amount of solute (outside the particle domain) of a given species from the concentration
  field \details \f[ N_\alpha^{\text{total}} = \int\vdf{r} (1 - \phi(r)) C_{\alpha}(\vec{r}) \f] \param[in] conc_x
  concentration field of a single species (r-space) \param[in] phi_p smooth particle field
 */
...
/*!
  \brief Compute the total amount of solute (outside the particle domain) for  all the species from the concentration
  fields \param[out] n_solute total solute amount for each species \param[in] conc_k concentration field for each
  species (k-space) \param[in] p particle data \param[in,out] phi working memory to compute smooth particle field

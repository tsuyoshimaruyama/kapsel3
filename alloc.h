...
/*!
\brief check the success of allocation
*/
void alloc_error_check(void *p);
...
*/
template <typename T> void free_1d(T *p) { _mm_free(p); p = NULL; }

/*!
\brief Create 2d view of 1d array referenced by i
\details Allocates array ii of size n1, each element of ii give view of rows
of i ii[I][J] = i[I*n2 + J]
*/
...
*/
template <typename T> void freeview_2d(T **pp) { free_1d<T *>(pp); }
...
\details Allocates array iii of size n1, each iii[.] points to 2D array ii
such that iii[I][J][K] = i[I*n2*n3 + J*n3 + K]
*/
template <typename T, typename Q>
void initview_3d(Q n1, Q n2, Q n3, T *const p, T ***const &ppp) {
	T **pp = ppp[0];
	pp[0] = p;
	for (Q j = Q(1); j < n1 * n2; j++)
		pp[j] = pp[j - 1] + n3;
}
...
void initview_3d_double(int n1, int n2, int n3, double *const d,
	double ***&ddd);

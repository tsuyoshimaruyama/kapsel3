...
void parallelepiped::distance_MI(const double r1[DIM], const double r2[DIM], double r12[DIM]) const {
...
void ewald::info(FILE* stream) const {
    ...
    fprintf(stream, "# k2_max            : %8g\n", k2max);
    fprintf(stream, "# k cut (kx, ky, kz): %d %d %d\n", kmax_l, kmax_m, kmax_n);
    fprintf(stream, "# rcut (r2max)      : %8g (%8g)\n", rcut, r2max);
    fprintf(stream, "# eta (eta * L)     : %8g (%8g) \n", eta, eta * rcut * 2);
    fprintf(stream, "#\n");
}
...
void ewald::init_domain_k(const double& ewald_delta, const double& ewald_conv) {
    double* tiH[DIM] = {cell->tiLambda[0], cell->tiLambda[1], cell->tiLambda[2]};
    ...
    //       kmax_l, kmax_m, kmax_n);
    {
        ...
        fprintf(kout, "# %d %12.6f %12.6f\n", ewald_domain, v_norm(ewald_k[0]), v_norm(ewald_k[ewald_domain - 1]));
...
void ewald::define_groups(const int* gid) {
    for (int i = 0; i < nump; i++) {
        group[i] = gid[i];
    }
}

void        ewald::reset(double* force, double* torque, double* efield, double* efield_grad) {
    ...
    for (int j = 1; j < nump; j++) {
        ...
        for (int i = 0; i < j; i++) {
            ...
            if (drij < r2max) {
                ...
                drij       = sqrt(drij);
                erfc_ewald = (iid != jid ? erfc(eta * drij) : -erf(eta * drij));
                ...
                if (DIPOLE) {
                    ...
                    if (CHARGE) {
                        ...
                        dmy_2 = -qi * Br;
                        for (int d = 0; d < DIM; d++)
                            dmy_force[d] += (dmy_0 * rij[d] + dmy_1 * mui[d] + dmy_2 * muj[d]);
                    }
...
void ewald::compute_force_externalfield_q(double*       force,
                                          const double* q,
                                          const double  efield_x,
                                          const double  efield_y,
                                          const double  efield_z) const {
    if (CHARGE) {
#pragma omp parallel for
        for (int i = 0; i < nump; i++) {
            const int     ii = i * DIM;
            const double* qi = &q[ii];
            // const double  inv_fourPI = 1.0/4.0/M_PI;
            force[ii] += qi[ii] * efield_x;
            force[ii + 1] += qi[ii] * efield_y;
            force[ii + 2] += qi[ii] * efield_z;
        }
    }
}
...
inline void ewald::compute_rho_k(double&       rho_re,
                                 double&       rho_im,
                                 double const* q,
                                 double const* mu,
                                 double const  kk[DIM],
                                 double const* coskr,
                                 double const* sinkr) const {
    rho_re = rho_im = 0.0;
...
void ewald::save_results(const double& E_ewald,
                         double const* force,
                         double const* torque,
                         double const* efield,
    ...
    for (int i = 0; i < nump; i++) {
        ...
        const int iii = ii * DIM;
        fprintf(fsave,
                "%20.12E %20.12E %20.12E %20.12E %20.12E %20.12E %20.12E %20.12E %20.12E %20.12E %20.12E %20.12E "
...
void ewald::save_results_cp2k(double const* E_ewald,
                              double const* force,
                              double const* torque,
                              double const* efield,
                              double const* efield_grad,
...
void ewald::compute(double*       E_ewald,
    ...
    // this->compute_force_externalfield_q(force, q, 0.1,0.0,0.0);//charge✖️efield

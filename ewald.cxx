...
void ewald::compute_self(double &energy, 
  ...
  // const double inv_fourPI = 1.0/4.0/M_PI;//wakiwaki
...
#pragma omp parallel for schedule(dynamic, 1) reduction(+:dmy_energy)
  for(int j = 1; j < nump; j++){
	...
	if(DIPOLE){
	  ...
	  //dipole - dipole          
	  dmy_energy += (Br*mui_muj - Cr*mui_r*muj_r);//wakiwaki
	  for(int d = 0; d < DIM; d++){
	    dmy_0 = rij[d] * Cr ;
	    dmy_1 = muj[d] * Cr ;
	    dmy_2 = mui[d] * Cr ;
...
//wakiwaki
void ewald::compute_force_externalfield_q(double* force, const double* q, const double efield_x, const double efield_y, const double efield_z) const{
  if(CHARGE){
    for(int i = 0; i < nump; i++){
      const int ii = i * DIM;

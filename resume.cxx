...
void Save_Rigid_Particle_udf() {
  if (SW_PT == rigid) {
    ...
    for (int rigidID = 0; rigidID < Rigid_Number; rigidID++) {
      ...
      for (int l = 0; l < DIM; l++)
        for (int m = 0; m < DIM; m++) ufres->put(target.sub(axis[l] + axis[m]), Rigid_Moments_body[rigidID][l][m]);
    }
  }
}
    ...
    for (int i = 0; i < NX; i++) {
      for (int j = 0; j < NY; j++) {
        for (int k = 0; k < NZ; k++) {
          ...
          K2[im] = SQ(WAVE_X * KX_int[im]) + SQ(WAVE_Y * KY_int[im] - WAVE_X * degree_oblique * KX_int[im]) +
                   SQ(WAVE_Z * KZ_int[im]);
          if (K2[im] > 0.0) {
            IK2[im] = 1.0 / K2[im];
          } else {
...
void Read_Rigid_Particle_udf(Particle *rigid_p) {
  if (SW_PT == rigid) {
    ...
    for (int rigidID = 0; rigidID < Rigid_Number; rigidID++) {
      ...
      string axis[DIM] = {"x", "y", "z"};
      for (int l = 0; l < DIM; l++)
        for (int m = 0; m < DIM; m++) ufin->get(target.sub(axis[l] + axis[m]), Rigid_Moments_body[rigidID][l][m]);
      rigid_body_matrix_rotation(
          Rigid_Moments[rigidID][0], Rigid_Moments_body[rigidID][0], rigid_p[rigidID].q, BODY2SPACE);

...
inline int Particle_cell(const double *xp, const double &dx, int *x_int, double *residue) {
	...
	for (int d = 0; d < DIM; d++) {
		assert((xp[d] >= 0) && (xp[d] < L_particle[d]));
		dmy = (xp[d] * idx);
		x_int[d] = (int)dmy;
 ...
 */
inline void Relative_coord(const int *cell, const int *x_int, const double *residue, const int &sw_in_cell
	, const int Nlattice[DIM], const double dx, int *r_mesh, double *r) {
	for (int d = 0; d < DIM; d++) {
		r_mesh[d] = (x_int[d] + cell[d] + Nlattice[d]) % Nlattice[d];
		assert((r_mesh[d] < Nlattice[d]) && (r_mesh[d] >= 0));
		r[d] = (double)cell[d] * dx - residue[d];
	}
}

//never used
inline void Relative_coord_OBL(const int *cell, const int *x_int, const double *residue
	, const int &sw_in_cell, const int Nlattice[DIM], const double dx, int *r_mesh, double *r) {
	...
	r_mesh[0] = (int)fmod(x_int[0] - sign*degree_oblique*Nlattice[1] + residue[0] / dx + cell[0] + 4 * Nlattice[0], Nlattice[0]);
	double x_oblique_residue = fmod(x_int[0] - sign*degree_oblique*Nlattice[1] + residue[0] / dx + cell[0] + 4 * Nlattice[0], Nlattice[0]) - r_mesh[0];
...
inline int Relative_coord_check_stepover_Y(const int *cell, const int *x_int, const double *residue
	, const int &sw_in_cell, const int Nlattice[DIM], const double dx, int *r_mesh, double *r) {
	...
	tmp = fmod(x_int[0] - sign*degree_oblique*Nlattice[1] + residue[0] / dx + cell[0] + 4 * Nlattice[0], Nlattice[0]);
	r_mesh[0] = (int)tmp;
	double x_oblique_residue = tmp - r_mesh[0];
	r_mesh[2] = (x_int[2] + cell[2] + Nlattice[2]) % Nlattice[2];
...
inline void Squirmer_coord(const double *x, double *r, double *theta, double *phi,
	...
	if (dmy_axis == z_axis) {
		...
		fprintf_single(stderr, "Error: Invalid Janus axis for Spherical_coord\n");
...
*/
inline void Reset_phi_primitive(double *phi, const int nx, const int ny,
	const int nz, const double &value) {
	int im;
...
#ifdef _MPI
	Reset_phi_primitive(phi, NPs[REAL][0], NPs[REAL][1], NPs[REAL][2], value);
#else
...
#ifdef _OPENMP
#pragma omp parallel default(none) shared(mesh_size, phi, up)
#endif
...
#ifdef _OPENMP
#pragma omp parallel default(none) shared(mesh_size, up)
#endif
...
inline void Reset_mesh(double *mesh) {
#pragma omp parallel for default(none) shared(THREADNUM, mesh_size, mesh)
	for (int i = 0; i < THREADNUM * mesh_size; i++) {
		mesh[i] = 0.0;
	}
}
#endif

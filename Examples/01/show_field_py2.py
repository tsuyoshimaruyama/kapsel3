...
if objType=="spherical_particle":
	...
	Ns=getArray($object_type.chain_polymer.Chain_spec[])
size_Ns=len(Ns)
RAD=($A*dx)*1.
...
def setAVS():
	avs_ts = str(int($t))
#	print os.path.join(join(os.path.split(_udf_.udfFile())[:-1]),$output.ON.Out_dir,"avs")
#	avs_file_path = join(os.path.join(os.path.split(_udf_.udfFile())[:-1])) + "/" + $output.ON.Out_dir + "/avs"
#	avs_file_path = os.getcwd() + "/" + $output.ON.Out_dir + "/avs"
	avs_file_path = os.path.join(join(os.path.split(_udf_.udfFile())[:-1]),$output.ON.Out_dir,"avs")

...
#pragma omp parallel for private(im)
  for (int i = 0; i < NX; i++) {
    for (int j = 0; j < NY; j++) {
      for (int k = 0; k < NZ; k++) {
        im = (i * NY * NZ_) + (j * NZ_) + k;
        potential[im] -= ((external[0] * (double)i + external[1] * (double)j + external[2] * (double)k) * DX);
      }
    }
  }
}
...
#pragma omp parallel for reduction(+ : rho_0) private(dmy_phi)
    ...
    for (int i = 0; i < NX; i++) {
      for (int j = 0; j < NY; j++) {
        for (int k = 0; k < NZ; k++) {
          ...
          Concentration[0][(i * NY * NZ_) + (j * NZ_) + k] =
              (1. - alp) * Concentration[0][(i * NY * NZ_) + (j * NZ_) + k] + alp * dmy;
          sum   = sum + fabs(dmy);
    ...
    A2a_k(Concentration[0]);
    {
      ...
      Rescale_solute(rescale_factor, Total_solute, Concentration, p, up[0], up[1]);
...
#pragma omp parallel for
  ...
  fprintf(stderr,
          "# density of positive and negative ions %g %g\n",

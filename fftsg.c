...
/*!
\brief Complex DFT (Discrete Fourier Transform)
    - definition
        - case1<BR>
    ...
    - usage
        - case1<BR>
            ip[0] = 0; // first time only <BR>
            cdft(2*n, 1, a, ip, w);
        - case2<BR>
            ip[0] = 0; // first time only<BR>
            cdft(2*n, -1, a, ip, w);
\param n              data length (int) <BR>
                        n >= 1, n = power of 2
\param a [0...2*n-1]  input/output data (double *) <BR>
                      input data <BR>
                            a[2*j] = Re(x[j]), <BR>
...
\param ip [0...*]     work area for bit reversal (int *)<BR>
                      length of ip >= 2+sqrt(n) <BR>
                      strictly, <BR>
                        length of ip >=
                            2+(1<<(int)(log(n+0.5)/log(2))/2).<BR>
                        ip[0],ip[1] are pointers of the cos/sin table.
\param w [0...n/2-1]  cos/sin table (double *)<BR>
                        w[],ip[] are initialized if ip[0] == 0.
\param isgn forward(1) / inverse (-1) transform
\remark
\code
        //Inverse of
            cdft(2*n, -1, a, ip, w);
...
/*!
  \brief Real DFT / Inverse of Real DFT
    - definition
        - case1 RDFT<BR>
            R[k] = sum_j=0^n-1 a[j]*cos(2*pi*j*k/n), 0<=k<=n/2 <BR>
            I[k] = sum_j=0^n-1 a[j]*sin(2*pi*j*k/n), 0<k<n/2
        - case2 IRDFT (excluding scale) <BR>
    ...
    - usage
     ...
     \param a [0...n-1] input/output data (double *)
                        - case1<BR>
                            output data<BR>
                                a[2*k] = R[k], 0<=k<n/2 <BR>
                                a[2*k+1] = I[k], 0<k<n/2 <BR>
                                a[1] = R[n/2]<BR>
                        - case2<BR>
                            input data<BR>
                                a[2*j] = R[j], 0<=j<n/2<BR>
                                a[2*j+1] = I[j], 0<j<n/2<BR>
                                a[1] = R[n/2]<BR>
      \param ip [0...*]  work area for bit reversal (int *)<BR>
                        length of ip >= 2+sqrt(n/2)<BR>
                        strictly,
                        length of ip >=
                            2+(1<<(int)(log(n/2+0.5)/log(2))/2).<BR>
                        ip[0],ip[1] are pointers of the cos/sin table.
      \param w [0...n/2-1]   cos/sin table (double *)<BR>
                        w[],ip[] are initialized if ip[0] == 0.
      \param isgn forward(1) / inverse (-1) transform
...
/*!
  \brief DCT (Discrete Cosine Transform) / Inverse of DCT
  \details
    - definition
    ...
    \param n            data length (int)<BR>
                        n >= 2, n = power of 2
    \param a [0...n-1]   input/output data (double *)<BR>
                        output data<BR>
                        a[k] = C[k], 0<=k<n
    \param ip [0...*]    work area for bit reversal (int *)<BR>
                        length of ip >= 2+sqrt(n/2)<BR>
                        ...
                        length of ip >=
                            2+(1<<(int)(log(n/2+0.5)/log(2))/2).<BR>
                        ip[0],ip[1] are pointers of the cos/sin table.
     \param w [0...n*5/4-1] cos/sin table (double *)<BR>
                        w[],ip[] are initialized if ip[0] == 0.
     \param isgn forward(1) / inverse (-1) transform
     \remark
     \code
...
/*!
\brief DST (Discrete Sine Transform) / Inverse of DST
    - definition
        - case1 IDST (excluding scale)<BR>
...
/*!
  \brief Cosine Transform of RDFT (Real Symmetric DFT)
  \details
    - definition<BR>
    ...
    - usage<BR>
        ip[0] = 0; // first time only<BR>
        dfct(n, a, t, ip, w);
    \param n            data length - 1 (int)<BR>
                        n >= 2, n = power of 2<BR>
    \param a [0...n]     input/output data (double *)<BR>
                        output data<BR>
                            a[k] = C[k], 0<=k<=n
    \param t [0...n/2]   work area (double *)<BR>
    \param ip [0...*]    work area for bit reversal (int *)<BR>
                        length of ip >= 2+sqrt(n/4)<BR>
                        strictly, <BR>
                        length of ip >=
                            2+(1<<(int)(log(n/4+0.5)/log(2))/2).<BR>
                        ip[0],ip[1] are pointers of the cos/sin table.
     \param w [0...n*5/8-1] cos/sin table (double *)<BR>
                        w[],ip[] are initialized if ip[0] == 0.
     \remark
     \code
        //Inverse of
...
/*!
  \brief Sine Transform of RDFT (Real Anti-symmetric DFT)
    - definition<BR>
    ...
    \param t [0...n/2-1] work area (double *)<BR>
    \param ip [0...*]    work area for bit reversal (int *)<BR>
                        length of ip >= 2+sqrt(n/4)<BR>
                        strictly, <BR>
                        length of ip >=

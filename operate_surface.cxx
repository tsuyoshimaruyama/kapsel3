...
void Make_particle_momentum_factor(double const* const* u, Particle *p){
  ...
  double slip_mode, slip_vel, slip_magnitude, C1, beta, slip_azimuthal;
...
#pragma omp parallel for \
  private(sw_in_cell, pspec, x_int, r_mesh, dmy_r, dmy_sr, dmy_phi, dmy_phi_s, xp, \
	  ...
	  n_r, n_theta, n_tau, slip_mode, slip_vel, slip_magnitude, C1, beta, slip_azimuthal, r_x_us, us, \
  ...
  for(int n = 0; n < Particle_Number; n++){
	...
	slip_magnitude = slip_vel * (sin(dmy_theta) + slip_mode * sin(2.0*sin(dmy_theta));
	slip_azimuthal = C1 * (sin(dmy_theta) + beta * 1.5 * (2.0*dmy_theta));
        for(int d = 0; d < DIM; d++){
...
void Make_force_u_slip_particle(double **up, double const* const* u, Particle *p, const CTime &jikan){
  ...
  double dmy_xi, dmy_theta, dmy_tau;
  double dmy_vslip, slip_mode, slip_vel, dmy_vslip2, C1, beta;
  double n_r[DIM], n_theta[DIM], n_tau[DIM];
...
#pragma omp parallel for \
  private(sw_in_cell, pspec, x_int, r_mesh, dmy_r, dmy_sr, dmy_phi, dmy_phi_s, xp, vp, omega_p, v_rot, \
	  ...
	  C1, beta, n_r, n_theta, n_tau, dmy_fv, force_s, torque_s, force_p, torque_p)
  for(int n = 0; n < Particle_Number; n++){
    pspec = p[n].spec;
...
#pragma omp atomic
	...
	if(less_than_mp(dmy_xi, HXI)){//interface domain
	  ...
	  dmy_vslip2 = C1 * (sin(dmy_theta) + beta * 1.5 * sin(2.0*dmy_theta));

...
#pragma omp parallel for private(sw_in_cell,     \
                                 pspec,          \
                                 x_int,          \
                                 r_mesh,         \
                                 dmy_r,          \
...
inline void slip_droplet(double* vp, double* wp, double* delta_v, double* delta_w, Particle p) {
    ...
    for (int d = 0; d < DIM; d++) {
        ...
        dL[d] = -(r_x_v[d] + p.surface_inertia[d][0] * wp[0] + p.surface_inertia[d][1] * wp[1] +
                  p.surface_inertia[d][2] * wp[2] + p.surface_dw[d]);
    }
...
#pragma omp atomic
                ...
                if (less_than_mp(dmy_xi, HXI)) {  // interface domain
                    ...
                    for (int d = 0; d < DIM; d++) {
                        dmy_fv[d] = dmy_phi_s *
                                    (vp[d] + v_rot[d] + n_theta[d] * dmy_vslip + n_tau[d] * dmy_vslip2 - u_fluid[d]);
                    }
...
#pragma omp atomic
            ...
            if ((v_rms(force_p, force_s) > LARGE_TOL_MP || v_rms(torque_p, torque_s) > LARGE_TOL_MP)) {
                fprintf(stderr, "###############################");
                fprintf(stderr,
                        "# Momentum Conservation Warning : %10.8E %10.8E\n",
                        v_rms(force_p, force_s),
                        v_rms(torque_p, torque_s));
                fprintf(stderr,
                        "# Force  : %10.8E %10.8E %10.8E %10.8E %10.8E %10.8E\n",
                        force_p[0],
                        force_p[1],
                        force_p[2],
                        force_s[0],
                        ...
                        force_s[2]);
                fprintf(stderr,
                        "# Torque : %10.8E %10.8E %10.8E %10.8E %10.8E %10.8E\n",
                        torque_p[0],
                        torque_p[1],
                        torque_p[2],
                        torque_s[0],

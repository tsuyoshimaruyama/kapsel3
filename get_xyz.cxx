...
#define NDIM 3
using namespace std;
enum OPTION {in_fin=1, in_fout=2};
...
void init_xyz(ofstream &outfile, char *fname);

void close_xyz(ofstream &outfile);


inline void wrong_invocation(){
  cout << "Usage: get_xyz UDFfile XYZfile" << endl;
  exit(1);
}
...
int main(int argc, char* argv[])
{
  ...
  }
  {
	...
	exit_job(EXIT_FAILURE);
      ...
      fprintf(stderr, "JANUS: %3.2f %3.2f %3.2f\n", 
	      janus_axis[i][0], janus_axis[i][1], janus_axis[i][2]);
      fprintf(stderr, "V_SLIP: %10.8g ", bslip[i]);
...
void write_xyz(ofstream &outfile, 
  ...
  // positions
  sprintf(dmy_str, "%.6g  %.6g  %.6g  %.6g  %.6g %.6g ", r[0], r[1], r[2], r_raw[0], r_raw[1], r_raw[2]);
  strcat(str, dmy_str);

...
inline double SGN(const double &a) { return a >= 0 ? 1. : -1.; }

inline bool equal_tol(const double &a, const double &b, const double &rtol, const double &atol = TOL_MP) {
  if (a == b) return true;
...
inline bool non_zero_mp(const double &a) { return (a > TOL_MP) || (-TOL_MP > a); }
inline bool zero_mp(const double &a) { return !non_zero_mp(a); }
inline bool positive_mp(const double &a) { return a > TOL_MP; }
inline bool negative_mp(const double &a) { return -TOL_MP > a; }
...
///
inline void exit_job(int status) { exit(status); }

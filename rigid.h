...
#pragma omp parallel for
  for (int rigidID = 0; rigidID < Rigid_Number; rigidID++) {
    PBC(xGs[rigidID]);
    for (int n = Rigid_Particle_Cumul[rigidID]; n < Rigid_Particle_Cumul[rigidID + 1]; n++) PBC(p[n].x);
  }
}
...
#pragma omp parallel for
  for (int rigidID = 0; rigidID < Rigid_Number; rigidID++) {
    ...
    for (int n = Rigid_Particle_Cumul[rigidID]; n < Rigid_Particle_Cumul[rigidID + 1]; n++) PBC_OBL(p[n].x, dmy_vx);
  }
}
...
#pragma omp parallel for private(rigidQ, rigid_first_n, rigid_last_n)
  for (int rigidID = 0; rigidID < Rigid_Number; rigidID++) {
    ...
    rigid_body_matrix_rotation(Rigid_Moments[rigidID][0], Rigid_Moments_body[rigidID][0], rigidQ, BODY2SPACE);
...
#pragma omp parallel for
    for (int rigidID = 0; rigidID < Rigid_Number; rigidID++) {
      ...
      {  // angular velocity
        ...
        double Ibody[DIM]  = {
            Rigid_Moments_body[rigidID][0][0], Rigid_Moments_body[rigidID][1][1], Rigid_Moments_body[rigidID][2][2]};
        int rigid_first = Rigid_Particle_Cumul[rigidID];

        MD_solver_omega_Euler(
            omegaGs[rigidID], torque, Ibody, p[rigid_first].q, Rigid_Motions_omega[rigid_spec], jikan.dt_md);
      }
    }
  } else if (CASE == "AB2") {
#pragma omp parallel for
    for (int rigidID = 0; rigidID < Rigid_Number; rigidID++) {
      ...
      {  // com velocity
        ...
        for (int d = 0; d < DIM; d++) {
          if (Rigid_Motions_vel[rigid_spec][d] == 0) continue;  // if "fix"
          velocityGs[rigidID][d] +=
              dmy * (2.0 * forceGs[rigidID][d] + forceGrs[rigidID][d] + forceGrs_previous[rigidID][d]);
        }
      }
      ...
      {  // angular velocity
        ...
        double torque_prev[DIM] = {
            torqueGrs_previous[rigidID][0], torqueGrs_previous[rigidID][1], torqueGrs_previous[rigidID][2]};
        double Ibody[DIM] = {
            Rigid_Moments_body[rigidID][0][0], Rigid_Moments_body[rigidID][1][1], Rigid_Moments_body[rigidID][2][2]};
        int rigid_first = Rigid_Particle_Cumul[rigidID];

        MD_solver_omega_AB2(omegaGs[rigidID],
                            torque,
                            torque_prev,
                            Ibody,
                            p[rigid_first].q,
                            p[rigid_first].q_old,
                            Rigid_Motions_omega[rigid_spec],
                            jikan.hdt_md);
      }
    }
  } else {

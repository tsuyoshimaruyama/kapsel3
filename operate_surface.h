...
inline void Update_slip_particle_velocity(Particle *p, const int &iter){
    if(iter == 0){
#ifdef _OPENMP
#pragma omp parallel for default(none) shared (Update_Particle_Number, p, iter)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared (Update_Particle_Number, p, iter, dmy_old, dmy_new)
#endif
        for(int n = 0; n < Update_Particle_Number; n++){
            for(int d = 0; d < DIM; d++){

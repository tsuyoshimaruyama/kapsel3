...
void Show_parameter(Particle *p){
  ...
  }
  {
    if(SW_PATCHY){
      ...
      fprintf(stderr, "# SOFT-CORE N = %s\n", powers[PATCHY_POWER]);
      fprintf(stderr, "# EPSILON = %10.6f (%10.6f)\n",  PATCHY_EPSILON, PATCHY_EPSILON/EPSILON);      
      fprintf(stderr, "# LAMBDA  = %10.6F (%10.6f)\n",  PATCHY_LAMBDA, PATCHY_LAMBDA*SIGMA);	
      fprintf(stderr, "# CUTOFF  = %10.6f (%10.6f)\n",  PATCHY_A_R_cutoff*SIGMA, PATCHY_A_R_cutoff);
      fprintf(stderr, "#\n");
	...
	for(int l = 0; l < max_points; l++){
	  ...
	  for(int nn = 0; nn <= npart; nn++){
	    ...
	    fprintf(FFORCE_B[nn], "%.6f", r_ij/SIGMA);
	    fprintf(FFORCE_F[nn], "%.6f", r_ij/SIGMA);
	    for(int d = 0; d < DIM; d++){
	      fprintf(FFORCE_B[nn], " %.6f", (-r_ij_vec[d]*dmy_r_bw - n_bw_vec[d]*dmy_o_bw)*FORCE_UNIT); 
	      fprintf(FFORCE_F[nn], " %.6f", (-r_ij_vec[d]*dmy_r_fw - n_fw_vec[d]*dmy_o_fw)*FORCE_UNIT);
	    }
	    fprintf(FFORCE_B[nn], "\n");
	    fprintf(FFORCE_F[nn], "\n");	    

	    fprintf(FTORQUE_B[nn], "%.6f", r_ij/SIGMA);
	    fprintf(FTORQUE_F[nn], "%.6f", r_ij/SIGMA);	    

	    fprintf(FTORQUE_B[nn], " %.6f %.6f\n",
